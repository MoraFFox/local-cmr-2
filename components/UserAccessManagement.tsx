...
const UserAccessManagement: React.FC = () => {
  ...
  const getStatusBadge = (status: InvitationStatus) => {
    switch (status) {
      ...
      case 'expired':
        return (
          <span className="inline-flex items-center rounded-full bg-surface-elevated dark:bg-chrome-light px-2.5 py-0.5 text-xs font-medium text-primary dark:text-secondary/70">
            {t.expired}
          </span>
        );
      default:
  ...
  const getDaysRemaining = (expiresAt: string) => {
    ...
    return `${days} ${days === 1 ? 'يوم' : days === 2 ? 'يومان' : 'أيام'}`;
  };
    ...
    <div className="w-full max-w-5xl mx-auto" dir="rtl">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <ArrowPathIcon className="w-8 h-8 text-brand-red animate-spin" />
        </div>
      ) : activeTab === 'invitations' ? (
        <>
          {/* Create Invitation Form */}
          {showForm && (
            <div className="bg-surface rounded-xl shadow-sm border border-default p-6 mb-6 animate-item-fade-in-down">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div className="bg-surface rounded-lg p-4 border border-default">
              ...
              <div className="text-sm text-secondary">{t.pendingCount}</div>
            </div>
            <div className="bg-surface rounded-lg p-4 border border-default">
              <div className="text-2xl font-bold text-leaf-500">{acceptedCount}</div>
              <div className="text-sm text-secondary">{t.acceptedCount}</div>
            </div>
            <div className="bg-surface rounded-lg p-4 border border-default">
              <div className="text-2xl font-bold text-secondary">{expiredCount}</div>
              <div className="text-sm text-secondary">{t.expiredCount}</div>
            </div>
          </div>
          ...
          {filteredInvitations.length === 0 ? (
            <div className="text-center bg-surface/80 backdrop-blur-sm p-12 rounded-2xl shadow-xl border border-default">
            ...
            <div className="space-y-3">
              {filteredInvitations.map((invitation) => {
                ...
                return (
                  <div
                    key={invitation.id}
                    className={`bg-surface rounded-lg shadow-sm border border-default p-4 ${
        ...
        <>
          ...
          {legacyLoading ? (
            ...
            <div className="text-center bg-surface/80 backdrop-blur-sm p-12 rounded-2xl shadow-xl border border-default">
              <DocumentTextIcon className="mx-auto h-12 w-12 text-secondary" />
              <h2 className="mt-4 text-lg font-semibold text-primary">
                {t.noLegacyRecords}
              </h2>
              <p className="text-secondary mt-2">
            ...
            <div>
              <div className="flex items-center gap-4 mb-6">
                ...
                <span className="inline-flex items-center rounded-full bg-surface-elevated px-2.5 py-0.5 text-xs font-medium text-secondary">
                  {selectedBucket.record_count} {t.records}
                </span>
              </div>

              {selectedSubmissionIds.length > 0 && (
                <div className="mb-4 p-4 bg-surface-elevated/50 rounded-lg border border-default">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <label className="flex items-center gap-2 text-sm text-primary">
                        <input
                          ...
                          className="rounded border-default bg-surface text-brand-red focus:ring-brand-red"
                        />
              ...
              <div className="bg-surface rounded-xl shadow-sm border border-default overflow-hidden">
                <table className="min-w-full divide-y divide-hairline">
                  <thead className="bg-surface-elevated/30">
                    <tr>
                      <th className="px-4 py-3 text-start text-xs font-medium text-secondary uppercase tracking-wider w-10">
                        <input
                          type="checkbox"
                      ...
                      <th className="px-4 py-3 text-start text-xs font-medium text-secondary uppercase tracking-wider">
                        {t.date}
                      </th>
                      <th className="px-4 py-3 text-start text-xs font-medium text-secondary uppercase tracking-wider">
                        {t.company}
            ...
            /* Bucket List View */
            <div>
              <div className="bg-surface rounded-xl shadow-sm border border-default p-6 mb-6">
          ...
          {showAssignModal && assigningBucket && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
              <div className="bg-surface rounded-xl shadow-xl border border-default p-6 w-full max-w-md animate-item-fade-in-down">
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div className="bg-surface dark:bg-chrome-light rounded-lg p-4 border border-default">
              ...
              <div className="text-sm text-primary dark:text-secondary/70">{t.totalUsers}</div>
            </div>
            <div className="bg-surface dark:bg-chrome-light rounded-lg p-4 border border-default">
              <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{adminCount}</div>
              <div className="text-sm text-primary dark:text-secondary/70">{t.adminUsers}</div>
            </div>
            <div className="bg-surface dark:bg-chrome-light rounded-lg p-4 border border-default">
              <div className="text-2xl font-bold text-brand-red dark:text-brand-red-400">{technicianCount}</div>
              <div className="text-sm text-primary dark:text-secondary/70">{t.technicianUsers}</div>
            </div>
          </div>
          ...
          {users.length === 0 ? (
            <div className="text-center bg-surface/80 backdrop-blur-sm p-12 rounded-2xl shadow-xl border border-default">
              <UsersIcon className="mx-auto h-12 w-12 text-secondary" />
              <h2 className="mt-4 text-lg font-semibold text-primary">
                {t.noUsers}
              </h2>
            </div>
                  ...
                  <tbody className="divide-y divide-hairline">
                    {users.map((user) => (
                      <tr key={user.id} className="hover:bg-surface-elevated/30">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-primary">
                          {user.name || <span className="text-secondary">—</span>}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm">
                          {user.hasTechnicianProfile ? (
                            ...
                            <span className="inline-flex items-center rounded-full bg-surface-elevated px-2 py-0.5 text-xs font-medium text-secondary">
                              {t.no}
                            </span>
                          )}
                        </td>
