...
const PartsSelector: React.FC<PartsSelectorProps> = ({
  ...
  useEffect(() => {
    ...
    if (count > 0) {
      announce(t.selectors.selectedPartsCount.replace('{{count}}', String(count)));
    }
    ...
    >
      ...
      <div className="flex gap-2">
        <div className="flex-grow" ref={searchInputRef}>
             <TechInput 
                 placeholder={t.selectors.searchParts}
                 value={searchTerm}
      ...
      <div className="bg-cream-2 border border-hairline rounded-2xl p-4 md:p-6">
        <h3 className="text-xs font-bold uppercase tracking-widest text-latte mb-4">{t.selectors.replacementsInventory}</h3>
        ...
        {filteredOptions.length > 0 ? (
          ...
          <div className="text-center py-12 text-latte">
             <p>{searchTerm ? t.selectors.noPartsMatch : t.payerFirstUI.noAvailableItems}</p>
          </div>
        )}
      </div>
            ...
            <div className="grid grid-cols-1 gap-4 animate-in fade-in slide-in-from-top-2">
                ...
                >
                    ...
                    onSubmit={async (item) => {
                      ...
                      showToast(t.selectors.partSavedToast, 'success');
