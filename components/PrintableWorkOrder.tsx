...
const LinedField: React.FC<{ label: string, className?: string }> = ({ label, className = '' }) => (
    <div className={`grid grid-cols-4 gap-x-2 items-end text-sm ${className}`}>
        <label className="text-slate-600 font-semibold text-right col-span-1 whitespace-nowrap">{label}:</label>
        <div className="border-b border-slate-400 border-dotted col-span-3 h-5"></div>
    </div>
);

const WorkItem: React.FC<{ label: string }> = ({ label }) => (
    <div className="p-1.5 border border-slate-300 rounded-md text-sm break-inside-avoid flex flex-col h-full">
        <p className="font-medium text-slate-800 flex-grow text-[13px]">{label}</p>
        <div className="flex justify-end items-center gap-x-2 mt-1.5 pt-1.5 border-t border-dotted border-slate-300">
            <div className="flex items-baseline gap-1">
                <label className="text-[11px] text-slate-500">Qty:</label>
                <div className="w-6 h-5 border-b border-slate-400"></div>
            </div>
            <div className="flex items-center gap-1">
                <label className="text-[11px] text-slate-500">Client Paid:</label>
                <div className="w-3.5 h-3.5 border border-slate-500 flex-shrink-0"></div>
            </div>
        </div>
    </div>
);
...
const CustomItemsTable: React.FC = () => (
    <div className="mt-4 break-inside-avoid">
        <h4 className="text-sm font-bold uppercase tracking-wider text-slate-700 mb-1.5">Custom Parts / Additional Services</h4>
                ...
                {Array.from({ length: 4 }).map((_, i) => (
                    <tr key={i} className="border-b border-dotted border-slate-300">
                        ...
                        <td className="p-1 flex justify-center items-center h-7">
                            <div className="w-4 h-4 border border-slate-500"></div>
                        </td>
                    </tr>
                ))}
            </tbody>
    ...
    return (
        <div className="space-y-2.5 p-1">
            {Array.from({ length: lines }).map((_, i) => (
                <div key={i} className="h-3 border-b border-slate-400 border-dotted"></div>
            ))}
        </div>
    );
};
...
const PrintableWorkOrder: React.FC<PrintableWorkOrderProps> = ({ onBack, partsList, servicesList }) => {
    ...
    return (
        <div className="bg-slate-200 dark:bg-slate-900 p-4 sm:p-8 print:p-0 print:bg-white">
            <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-3 print:hidden">
                <button
                    onClick={() => window.print()}
                    className="flex items-center gap-2 bg-teal-600 text-white font-bold py-3 px-5 rounded-full hover:bg-teal-700 transition-colors shadow-lg transform active:scale-95"
                >
                 ...
                 <button
                    onClick={onBack}
                    className="flex items-center gap-2 bg-slate-600 text-white font-bold py-3 px-5 rounded-full hover:bg-slate-700 transition-colors shadow-lg transform active:scale-95"
                >
            ...
            <div className="max-w-4xl mx-auto bg-white p-4 sm:p-8 shadow-sm rounded-lg font-sans text-slate-800 print:shadow-none print:rounded-none">
                ...
                <header className="flex justify-between items-center pb-3 border-b-2 border-slate-700">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800">Maintenance Visit Report</h1>
                        <p className="text-xs text-slate-500">Internal Use Document</p>
                    </div>
                ...
                <main className="mt-6 space-y-4">
                    <div className="p-2 border border-slate-300 rounded-md">
                        ...
                        <div className="mb-3 break-inside-avoid">
                            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">Parts Used</h4>
                    ...
                    <div>
                        <h3 className="text-sm font-bold uppercase tracking-wider text-slate-700 bg-slate-200 p-2 rounded-t-md border-b-2 border-slate-400">Identified Issues, Recommendations & Notes</h3>
                        <div className="p-1 border border-t-0 border-slate-300 rounded-b-md">
                ...
                {/* Footer */}
                <footer className="mt-12 pt-6 border-t-2 border-slate-700 text-center">
                    <p className="text-xs text-slate-500">End of Work Order</p>
                </footer>
            </div>
            <style>{`
                @media print {
