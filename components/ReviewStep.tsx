...
const MaintenanceRecordReview: React.FC<{
  ...
  return (
    <div
      className={`p-3 my-2 border border-default rounded-md ${isFollowUp ? "bg-surface-elevated" : "bg-surface-3"}`}
    >
      <div className="flex justify-between items-center">
        ...
        {record.nextVisitDate && (
          <span className="text-xs bg-surface-elevated text-espresso dark:bg-chrome dark:text-cream px-2 py-1 rounded-full font-medium">
      ...
      <dl className="mt-2 text-xs divide-y divide-hairline">
            ...
            value={
              <ul className="space-y-2">
                {record.supervisors.map((s) => (
                  <li
                    key={s.id}
                    className="border-l-2 pl-2 border-default"
                  >
        ...
        {record.followUpVisits && record.followUpVisits.length > 0 && (
          <div className="pl-4 mt-2 pt-2 border-t border-default">
            <h6 className="text-sm font-semibold text-secondary">
              Follow-up Visits:
            </h6>
...
const ReviewStep: React.FC<ReviewStepProps> = ({
  ...
  const Content = () => (
    <div className="space-y-8">
      {/* Main Company Info */}
      <section>
        <h3 className="text-lg font-semibold text-brand-red border-b border-default pb-2 mb-3">
          Company Details
        </h3>
        ...
        <section>
          ...
          <div className="space-y-4">
            {formData.branches.map((branch, index) => (
              <div
                key={branch.id}
                className="p-4 border border-default rounded-lg bg-surface-elevated"
              >
                  ...
                  <div className="mt-4 pt-3 border-t border-default">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                      {branch.baristas.map((barista) => (
                        <div
                          key={barista.id}
                          className="flex flex-col items-center text-center p-2 rounded-lg bg-surface shadow-sm border border-default/50"
                        >
                          ...
                          <div className="flex mt-1">
                            ...
                            {Array.from({ length: 5 - barista.rating }).map(
                              (_, i) => (
                                <StarIcon
                                  key={i}
                                  className="h-4 w-4 text-hairline"
                                />
                              ),
                            )}
                          </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {formData.baristas.map((barista) => (
              <div
                key={barista.id}
                className="flex flex-col items-center text-center p-2 rounded-lg bg-surface"
              >
                <Avatar name={barista.name} />
                <p className="font-medium text-primary text-sm mt-2">
                  {barista.name}
            ...
            <div className="mt-4 p-4 rounded-lg bg-surface-elevated/50 dark:bg-brand-red/10">
              <div className="py-2 sm:grid sm:grid-cols-3 sm:gap-4 items-center">
                ...
                <dd className="mt-1 text-primary sm:col-span-2 sm:mt-0 flex justify-between items-center">
                  <span className="font-bold text-brand-red text-base">
                    {formatCurrency(totalMaintenanceCost)}
                  </span>
                  {totalMaintenanceCost > 0 && (
                    <button
                      onClick={() => setIsCostModalOpen(true)}
                      className="text-sm font-semibold text-brand-red hover:text-brand-red transition-colors transform active:scale-95"
                    >
                      View Details
                    </button>
                  )}
