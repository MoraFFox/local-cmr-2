...
const TechnicianInvitations: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'expired':
        return (
          <span className="inline-flex items-center rounded-full bg-surface dark:bg-chrome-light px-2.5 py-0.5 text-xs font-medium text-primary dark:text-secondary/70">
            Expired
          </span>
        );
      default:
    ...
    <div className="w-full max-w-5xl mx-auto">
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-primary dark:text-secondary/70 mt-1 sm:mt-2">
      ...
      {showForm && (
        <div className="bg-surface dark:bg-chrome-light rounded-xl shadow-sm border border-default p-6 mb-6 animate-item-fade-in-down">
          <h2 className="text-lg font-semibold text-primary dark:text-white mb-4">
            Send New Invitation
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="tech-name" className="block text-sm font-medium text-primary dark:text-secondary/70 mb-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="tech-email" className="block text-sm font-medium text-primary dark:text-secondary/70 mb-1">
                  Email (Optional)
                </label>
                <input
                  id="tech-email"
              ...
              <div>
                <label htmlFor="tech-phone" className="block text-sm font-medium text-primary dark:text-secondary/70 mb-1">
                  Phone (Optional)
                </label>
                <input
                  id="tech-phone"
            ...
            <p className="text-xs text-secondary dark:text-secondary/70">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-primary dark:text-secondary/70 bg-surface dark:bg-chrome-light rounded-lg hover:bg-surface-elevated dark:hover:bg-chrome-light/50 transition-colors"
              >
                Cancel
              </button>
              <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-surface dark:bg-chrome-light rounded-lg p-4 border border-default">
          <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">{pendingInvites.length}</div>
          <div className="text-sm text-primary dark:text-secondary/70">Pending</div>
        </div>
        <div className="bg-surface dark:bg-chrome-light rounded-lg p-4 border border-default">
          <div className="text-2xl font-bold text-leaf-600 dark:text-leaf-500">{acceptedInvites.length}</div>
          <div className="text-sm text-primary dark:text-secondary/70">Accepted</div>
        </div>
        <div className="bg-surface dark:bg-chrome-light rounded-lg p-4 border border-default">
          <div className="text-2xl font-bold text-primary dark:text-secondary/70">{expiredInvites.length}</div>
          <div className="text-sm text-primary dark:text-secondary/70">Expired</div>
        </div>
      </div>

      {/* Invites List */}
      {invites.length === 0 ? (
        <div className="text-center bg-surface/80 dark:bg-chrome-light/80 backdrop-blur-sm p-12 rounded-2xl shadow-xl border border-black/5 dark:border-white/5">
          <UserPlusIcon className="mx-auto h-12 w-12 text-secondary" />
          <h2 className="mt-4 text-lg font-semibold text-primary dark:text-secondary/70">
            No invitations yet
          </h2>
          <p className="text-secondary dark:text-secondary/70 mt-2">
        ...
        <div className="space-y-4">
          ...
          {pendingInvites.length > 0 && (
            <>
              <h3 className="text-lg font-semibold text-primary dark:text-secondary/70 mt-6 mb-3">
                Pending Invitations
              </h3>
              {pendingInvites.map(invite => (
                <div
                  key={invite.id}
                  className="bg-surface dark:bg-chrome-light rounded-lg shadow-sm border border-default p-4"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-secondary dark:text-secondary/70 mt-1">
                          {invite.email && <span className="mr-3">{invite.email}</span>}
                          {invite.phone && <span>{invite.phone}</span>}
                        </div>
                        <div className="text-xs text-secondary dark:text-secondary/70 mt-1">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <button
                        ...
                        className="p-2 text-secondary hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-colors"
                      ...
                      <button
                        ...
                        className="p-2 text-secondary hover:text-ember-700 dark:hover:text-ember-300 hover:bg-ember-50 dark:hover:bg-ember-500/10 rounded-lg transition-colors"
          ...
          {acceptedInvites.length > 0 && (
            <>
              ...
              {acceptedInvites.map(invite => (
                <div
                  key={invite.id}
                  className="bg-surface dark:bg-chrome-light rounded-lg shadow-sm border border-default p-4 opacity-75"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex items-start gap-3">
          ...
          {expiredInvites.length > 0 && (
            <>
              ...
              {expiredInvites.map(invite => (
                <div
                  key={invite.id}
                  className="bg-surface dark:bg-chrome-light rounded-lg shadow-sm border border-default p-4 opacity-60"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex items-start gap-3">
                      <div className="h-10 w-10 rounded-full bg-surface-elevated dark:bg-chrome-light flex items-center justify-center">
                        <ClockIcon className="h-5 w-5 text-secondary" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-semibold text-primary dark:text-secondary/70">{invite.name}</h4>
                          {getStatusBadge(invite.status)}
                        </div>
                        <div className="text-sm text-secondary dark:text-secondary/70 mt-1">
                          {invite.email && <span className="mr-3">{invite.email}</span>}
