...
const CollapsibleCard: React.FC<CollapsibleCardProps> = ({ titleContent, children, initiallyOpen = false, onRemove, removeLabel = 'Remove' }) => {
    ...
    return (
        <div className={`border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 shadow-md transition-shadow duration-300 ${isOpen ? 'shadow-lg' : 'shadow-md'}`}>
            <div 
                className={`flex justify-between items-center p-4 cursor-pointer rounded-t-xl ${!isOpen ? 'rounded-b-xl' : ''}`} 
                onClick={() => setIsOpen(!isOpen)}
            >
                <div className="flex-grow font-semibold text-slate-800 dark:text-slate-200 relative">
                    {titleContent}
                </div>
                <div className="flex items-center gap-2 sm:gap-4 pl-2">
                    {onRemove && (
                        <button 
                            ...
                            className="text-xs font-semibold text-slate-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-400 transition-colors transform active:scale-95"
