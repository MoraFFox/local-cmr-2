...
const SidebarContent = React.memo(({ 
  ...
  return (
    <>
      <div
        className={`flex items-center h-16 border-b border-slate-200 dark:border-slate-800 shrink-0 transition-all duration-300 ${isSidebarExpanded ? "justify-start px-6 gap-3" : "justify-center overflow-hidden"}`}
      >
        <img src="/logo.svg" alt="شعار ميدوز" className="h-8 w-auto object-contain" />
        <h1
          className={`text-xl font-bold transition-opacity duration-200 whitespace-nowrap text-brand-espresso dark:text-white ${isSidebarExpanded ? "opacity-100" : "opacity-0 w-0 overflow-hidden"}`}
        >
          ميدوز
        </h1>
      </div>
      <nav className="flex-grow p-2 space-y-1">
        <div className="px-2 pb-2">
          <button
            ...
            className="w-full flex items-center gap-2 p-2 rounded-lg text-sm font-bold bg-success-600 hover:bg-success-700 text-white transition-colors active:scale-[0.98]"
        ...
        {NAV_ITEMS.filter((n) => n.inSidebar).map((item) => {
          ...
          return (
            <button
              ...
              className={`flex items-center w-full gap-3 p-2 rounded-md text-sm font-medium transition-colors overflow-hidden ${
                isActive
      ...
      <div className="p-2 border-t border-slate-200 dark:border-slate-800 shrink-0 space-y-2">
        {/* Drafts Section */}
        {drafts.length > 0 && (
        ...
        {onAdminLogout && (
          <button
            ...
            className={`w-full flex items-center gap-3 p-3 rounded-md text-sm font-semibold border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors ${!isSidebarExpanded && "justify-center"}`}
            title="تسجيل الخروج"
          >
        ...
        <button
          ...
          className={`w-full hidden lg:flex items-center p-2 rounded-md text-sm font-medium text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 transition-colors overflow-hidden ${!isSidebarExpanded && "justify-center"}`}
